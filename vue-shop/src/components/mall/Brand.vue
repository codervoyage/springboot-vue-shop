                ...
                <el-col :span="22" :offset="1">
                    <el-table id="table"
                        ...
                        <el-table-column label="操作">
                            <template v-slot="scope">
                                <el-button type="primary" size="mini" @click="showBrandByDiaLog(scope.row)">编辑
                                </el-button>
                                <el-button type="danger" @click="deleteInfo(scope.row.bId)" size="mini">删除</el-button>
                            </template>
                        </el-table-column>
                    </el-table>
                </el-col>
...
<script>
  export default {
    ...
    methods: {
      ...
      //删除方法
      deleteInfo (id) {
        this.$confirm('确认删除吗？', { type: 'error' }).then(async _ => {
      ...
      async updateUploadAction (uploadParams) {
        ...
        if (!Number.isNaN(low)) {
          const formData = new FormData()
          formData.append('file', uploadParams.file)
          formData.append('id', this.updateForm.bId)
